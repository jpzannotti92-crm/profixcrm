...
try {
    ...
    echo "=== AGREGAR PERMISO leads.view AL ROL Sales Agent ===\n";
    ...
    // 2. Obtener el ID del permiso leads.view
    $stmt = $db->prepare("SELECT id FROM permissions WHERE name = 'leads.view'");
    $stmt->execute();
    ...
    if ($existingRelation) {
        ...
        $stmt->execute([$roleId, $permissionId]);
        echo "✅ Permiso 'leads.view' agregado al rol 'Sales Agent'\n";
    }
    ...
    $stmt = $db->prepare("
        ...
        WHERE rp.role_id = ? AND p.name = 'leads.view'
    ");
    $stmt->execute([$roleId]);
    $verification = $stmt->fetch();
    
    if ($verification) {
        echo "✅ Verificación exitosa: El rol 'Sales Agent' ahora tiene el permiso 'leads.view'\n";
    } else {
